...
{/loop}
"{/if}>{if $uri}<a href="/{$_.game}/{$uri}">{/if}{if $name}{string_format($name, defaultv($printformat))}{else}{string_format($value, defaultv($printformat))}{/if}{if $uri}</a>{/if}</span>{else}<a class="label" href="#{$label}" name="{$label}">{$label}</a>:{/if}
{/loop}{/block}