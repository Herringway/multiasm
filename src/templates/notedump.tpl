{%extends "base.tpl"%}
{%block content%}
{%for entry in data%}<a href="{{rootdir}}/{{coremod}}/{{entry.name}}">${{addrformat|format(entry.address)}} to ${{addrformat|format(entry.address+entry.size-1)}}</a> = {%if entry.description%}{{entry.description|trim()}}{%else%}Unknown{%endif%}{%if entry.notes%}
